...
/// 适配器的创建、缓存与搜索/播放调用入口。
///
/// 只读的源配置查询请直连 [sourceCatalog]；本服务保留会牵动
/// 适配器缓存失效的变更操作（增删改源、导入、重置）。
/// 所有调用方共享一个适配器 LRU，避免重复缓存 Cookie、DOM 和请求连接。
late SourceAdapterService sourceRepository;
late SourceCatalog sourceCatalog;

class SourceAdapterService {
  SourceAdapterService(this._catalog) {
  ...
  Future<void> init() => _initialization ??=
      Future.wait<void>([
        BundledRuleStore.load(),
        _catalog.init(),
      ]).then((_) {}).catchError((Object error, StackTrace stack) {
...
class SourceCatalog extends ChangeNotifier {
  ...
  SourceCatalog(this.preferences) {
    _disabledKeys = preferences.getStringList(_disabledKeysKey)?.toSet() ?? {};
    final usedKeys = <String>{};
    final ordered = <AdapterDescriptor>[];
    for (final key
        in preferences.getStringList(_orderKeysKey) ?? const <String>[]) {
  ...
  Future<void> enableAllBuiltins() async {
    ...
    _disabledKeys.clear();
    _rebuildBuiltinViews();
    await preferences.setStringList(_disabledKeysKey, const <String>[]);
  }
  ...
  Future<void> init() => _initialization ??= _loadSources().catchError((
    Object error,
    StackTrace stack,
  ) {
    _initialization = null;
  ...
  Future<void> _loadSources() async {
    ...
    if (staleVersionKeys.isNotEmpty) {
      await Future.wait(staleVersionKeys.toSet().map(preferences.remove));
    }
    if (storageChanged) {
      await _commit();
      return;
