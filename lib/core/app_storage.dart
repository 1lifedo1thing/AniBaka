...
class AppStorage {
  ...
  static const startupBoxes = {
    homeCacheBoxName,
    bgmCacheBoxName,
    playHistoryBoxName,
    videoProgressBoxName,
  };
  static Directory? _directory;
  static final _opening = <String, Future<void>>{};
  static const _listBoxes = {
    customSourcesBoxName,
    downloadTasksBoxName,
    playHistoryBoxName,
  };

  static Future<List<HiveBoxRecovery>> init({
    Directory? hiveDirectory,
    Set<String>? boxes,
  }) async {
    _directory = hiveDirectory;
    ...
    await Future.wait(
      (boxes ??
              const {
                ...
                downloadTasksBoxName,
                playHistoryBoxName,
                threadCommentsBoxName,
                bgmCacheBoxName,
                homeCacheBoxName,
              })
          .map((name) => open(name, recoveries: recoveries)),
    );
    return recoveries;
  }

  static Future<void> open(String name, {List<HiveBoxRecovery>? recoveries}) {
    if (Hive.isBoxOpen(name)) return Future.value();
    return _opening.putIfAbsent(
      name,
      () => Future<void>(() async {
        final recovered = recoveries ?? <HiveBoxRecovery>[];
        try {
          if (_listBoxes.contains(name)) {
            await _openBox<List>(name, _directory, recovered);
          } else {
            await _openBox<Map>(name, _directory, recovered);
          }
          for (final item in recovered) {
            debugPrint('Recovered Hive box ${item.boxName}: ${item.reason}');
          }
        } finally {
